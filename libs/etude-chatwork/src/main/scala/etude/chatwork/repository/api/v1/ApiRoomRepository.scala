...
import scala.Some
import etude.chatwork.model.RoomAttributes
import scala.util.Success
import etude.chatwork.repository.{RoomRepository, EntityNotFoundException, AdminAccountRequiredException}

case class ApiRoomRepository(implicit authToken: TokenAuthentication) extends RoomRepository with ApiQoS {
      ...
      Api.put(
        ...
        case Success(json) =>
          ...
          result.lastOption match {
            ...
            case _ => Failure(ApiException("Failed to create room"))
          }
      }
    } finally {
